...
export function computeMarketing(data, office = null) {
  ...
  // Helper to parse Year + Month into a date
  const parseYearMonth = (year, month) => {
    ...
    if (monthNum === undefined) {
      // Try parsing as date string (some rows have full dates)
      const parsed = new Date(month);
      if (!isNaN(parsed.getTime())) return parsed;
      return null;
    }
    return new Date(year, monthNum, 15); // Use 15th of month
  };

  // Recent wins (awarded in last 90 days, using Year + Submitted/Awarded)
  const now = Date.now();
  ...
  // Recent losses (NA in last 90 days, based on Year + Submitted)
  const losses = proposalLog
    .filter(row => {
