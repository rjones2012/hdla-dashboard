...
export async function loadAllData(forceRefresh = false) {
  ...
  const proposalLog = parseSheet(masterWb, 'Proposal Log');
...
export function computeMarketing(data, office = null) {
  ...
  // Recent wins (awarded in last 90 days)
  const now = Date.now();
  const ninetyDaysAgo = now - (90 * 24 * 60 * 60 * 1000);
  const wins = proposalLog
    .filter(row => {
      if (row.Status !== 'A') return false;
      const awarded = row.Awarded ? new Date(row.Awarded).getTime() : 0;
      return awarded >= ninetyDaysAgo;
    })
    .map(row => ({ ...row, fee: parseNumeric(row.Fee) }))
    .sort((a, b) => new Date(b.Awarded) - new Date(a.Awarded));
  
  // Recent losses (NA or D in last 90 days, based on Submitted date as proxy)
  const losses = proposalLog
    .filter(row => {
      if (row.Status !== 'NA' && row.Status !== 'D') return false;
      const submitted = row.Submitted ? new Date(row.Submitted).getTime() : 0;
      return submitted >= ninetyDaysAgo;
    })
    .map(row => ({ ...row, fee: parseNumeric(row.Fee) }))
    .slice(0, 20);
