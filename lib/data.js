...
import { fetchCapsuleActivity, daysSinceTouch } from './capsule';
import { 
  PROB_MAP, 
...
// Marketing metrics
export async function computeMarketing(data, office = null) {
  ...
  // --- Flatten multi-market clients into one row per client ---
  const criticalMarkets = ['Parks', 'Campus', 'Mixed-Use', 'Civic', 'State'];
  const clientGroups = {};
  clients.forEach(row => {
    const name = row.Client;
    if (!clientGroups[name]) clientGroups[name] = [];
    clientGroups[name].push(row);
  });

  const flatClients = Object.entries(clientGroups).map(([name, rows]) => {
    if (rows.length === 1) {
      // Single-market client — pass through with market metadata
      const row = rows[0];
      const market = row.Market || '';
      return {
        ...row,
        markets: [market],
        criticalMarketCount: criticalMarkets.includes(market) ? 1 : 0,
        multiPartner: false,
        allPartners: [row.Partner].filter(Boolean),
      };
    }

    // Multi-market client — flatten
    // Best (lowest) tier wins for display
    const bestTierRow = rows.reduce((best, r) => {
      const t = parseInt(r.Tier) || 5;
      const bestT = parseInt(best.Tier) || 5;
      return t < bestT ? r : best;
    }, rows[0]);

    // Collect all markets and count critical ones
    const allMarkets = [...new Set(rows.map(r => r.Market).filter(Boolean))];
    const critCount = allMarkets.filter(m => criticalMarkets.includes(m)).length;

    // Collect all partners — flag if multiple
    const allPartners = [...new Set(rows.map(r => r.Partner).filter(Boolean))];
    const multiPartner = allPartners.length > 1;

    // Use best-tier row's relationship/touchpoint scores
    // Market display: join all markets
    return {
      ...bestTierRow,
      Market: allMarkets.join(', '),
      markets: allMarkets,
      criticalMarketCount: critCount,
      multiPartner,
      allPartners,
    };
  });

  // --- Enrich flattened clients with project data ---
  const enriched = flatClients.map(client => {
    ...
    // Critical market bonus: flat +8 if in any critical market
    const critCount = client.criticalMarketCount || 0;
    const criticalBonus = critCount > 0 ? 8 : 0;
    
    let priority = (6 - tier) * 2.0;
    priority += traction * 0.25;
    priority += criticalBonus;
    ...
    if (client.multiPartner) flags.push('Multi-partner');
  ...
  // --- Merge Capsule CRM activity ---
  let capsuleActivity = null;
  try {
    capsuleActivity = await fetchCapsuleActivity();
  } catch (err) {
    console.warn('[Marketing] Capsule fetch failed, continuing without:', err.message);
  }
  ...
  if (capsuleActivity) {
    ...
    for (const client of enriched) {
      const clientName = String(client.Client || '').trim();
      const activity = capsuleActivity[clientName];
      
      if (activity) {
        matched++;
        client.capsule = {
          lastClosed: activity.lastClosed,
          lastClosedName: activity.lastClosedName,
          lastClosedOwner: activity.lastClosedOwner,
          nextOpenName: activity.nextOpenName,
          nextOpenOwner: activity.nextOpenOwner,
          openCount: activity.openCount,
          closedCount: activity.closedCount,
          daysSinceTouch: daysSinceTouch(activity.lastClosed),
        };
      } else {
        client.capsule = null;
      }
    ...
    // Log unmatched Capsule orgs (helps with reconciliation)
    const enrichedNames = new Set(enriched.map(c => String(c.Client || '').trim()));
    const capsuleOrgs = Object.keys(capsuleActivity);
    const unmatchedOrgs = capsuleOrgs.filter(name => !enrichedNames.has(name));
    
    console.log(`[Marketing] Capsule match: ${matched}/${enriched.length} clients matched`);
    if (unmatchedOrgs.length > 0) {
      console.log(`[Marketing] Capsule orgs not in Marketing.xlsx: ${unmatchedOrgs.join(', ')}`);
    }
  } else {
    // No Capsule data — set null on all clients
    for (const client of enriched) {
      client.capsule = null;
    }
  }
