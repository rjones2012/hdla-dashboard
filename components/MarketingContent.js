...
export default function MarketingContent({ office = null }) {
  const [analysisTab, setAnalysisTab] = useState('partner');
  const { data, loading, error, refetch } = useMarketingData(office);
  ...
  // Strategic targets (flagged as strategic - column may have trailing space)
  const strategic = allClients.filter(c => {
    const val = c.Strategic || c['Strategic '] || '';
  ...
  const title = office ? `Marketing ${office} — Action Board` : 'Marketing — Action Board';
